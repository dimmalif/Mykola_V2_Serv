...
class Search_music(BFunction):
    ...
    def search_top_result(self):
        yt = YTMusic()
        search_results = yt.search(self.request[1])
        print(self.request)
        print(f"{search_results} - результат пошуку")
...
class Downloader_music(Search_music):
    def __init__(self, requests=None):
        ...
        self.file_list = []
        self.name = []
        self.VIDEO_FILE_PATH = "/home/dmytro/Desktop/Mykola/Bot_Mykola"
        self.AUDIO_FILE_PATH = "/home/dmytro/Desktop/Mykola/Skills/downloader_music/mp3_files"
    ...
    def renamed_downloads_file(self, user_id=None):
        if user_id:
            self.name = [user_id]
        # print(self.name)
...
# d = Downloader_music('оля скечей пісню гимн панков')
